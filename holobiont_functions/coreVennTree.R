    ...
    if (is.null(branch_color)){
      ...
      if (length(branch_color)<(1+length(combos))){
        ...
        if (plot.chronogram==FALSE){
          #plot the tree
          if (scaled == FALSE){
          tt<-plot.phylo(phy_tree(x),edge.color=branch_color[states],edge.width = edge_width,show.tip.label=tip_label)
          }else{
            temptree<-phy_tree(x)
            temptree$edge.length <- sqrt(temptree$edge.length+1e-2)
            tt<-plot.phylo(temptree,edge.color=branch_color[states],edge.width = edge_width,show.tip.label=tip_label)
          }
        }else{
          tt<-plot.phylo(chronos(phy_tree(x)),edge.color=branch_color[states],edge.width = edge_width,show.tip.label=tip_label)
        }
