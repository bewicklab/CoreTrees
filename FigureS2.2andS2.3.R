...
list<-coreEdges(pps,selection='shade',max_tax=20, increase_cutoff = 20,initial_branches = 2,remove_zeros=FALSE,core_fraction=0.49)
...
coreRichASV<-c(coreRichASV,coreRichness(pps,max_tax=20, increase_cutoff = 20,core_fraction = 0.49,selection='shade'))
coreFaithASVt<-c(coreFaithASVt,coreFaithsPD(pps,max_tax=20, increase_cutoff = 20,core_fraction = 0.49,mode='tip',selection='shade'))
coreFaithASV<-c(coreFaithASV,coreFaithsPD(pps,max_tax=20, increase_cutoff = 20,core_fraction = 0.49,selection='shade',initial_branches=2))
tt<-coreTaxa(pps,selection='shade',max_tax=20, increase_cutoff = 20,core_fraction=0.49)
bb<-rep('black',length(phy_tree(pps)$tip.label))
bb[which(phy_tree(pps)$tip.label %in% tt)]<-'red'
temptree<-phy_tree(pps)
temptree$edge.length <- sqrt(temptree$edge.length+1e-2)
plot(phy_tree(temptree),cex=0.3,tip.color=bb)
...
coreRichOTU<-c(coreRichOTU,coreRichness(pps,max_tax=20, increase_cutoff = 20,core_fraction = 0.49,selection='shade'))
coreFaithOTUt<-c(coreFaithOTUt,coreFaithsPD(pps,max_tax=20, increase_cutoff = 20,core_fraction = 0.49,mode='tip',selection='shade'))
coreFaithOTU<-c(coreFaithOTU,coreFaithsPD(pps,max_tax=20, increase_cutoff = 20,core_fraction = 0.49,selection='shade',initial_branches=2))
tt<-coreTaxa(pps,selection='shade',max_tax=20, increase_cutoff = 20,core_fraction=0.49)
bb<-rep('black',length(phy_tree(pps)$tip.label))
bb[which(phy_tree(pps)$tip.label %in% tt)]<-'red'
temptree<-phy_tree(pps)
...
ggsave("StaphFigureS2.2.png", grid_layout, height = 20, width = 30)
...
coreVennTree(ppb,sample_data(ppb)$Habitat,core_fraction=0.49,scaled=TRUE,selection='shade',initial_branches=2,increase_cutoff = 20, max_tax = 20,branch_color=c('black','khaki','magenta','tan3','royalblue1','green','magenta','darkgoldenrod1','magenta','yellowgreen','magenta','magenta','magenta','lightsteelblue2','magenta','magenta','magenta','grey','grey','grey','cyan3','grey','lightyellow3','grey','grey','grey','cyan','steelblue',rep('magenta',3),'red'),edge_width=3)
coreVennTree(ppb,sample_data(ppb)$Habitat,core_fraction=0.49,scaled=TRUE,selection='shade',increase_cutoff=20,max_tax=20,mode='tip',branch_color=c('black','khaki','grey','grey','royalblue1','green','grey','darkgoldenrod1','grey','yellowgreen','grey','grey','grey','lightsteelblue2','grey','grey','grey','grey','grey','grey','grey','grey','grey','grey','grey','grey','cyan','steelblue',rep('magenta',3),'red'),edge_width=3)
...
points(1,5,pch=16,col='tan3')
points(1,6,pch=16,col='yellowgreen')
points(1,7,pch=16,col='lightsteelblue2')
points(1,8,pch=16,col='cyan3')
points(1,9,pch=16,col='lightyellow3')
points(1,10,pch=16,col='cyan')
points(1,11,pch=16,col='steelblue')
points(1,12,pch=16,col='red')
...
for (k in 1:5){
  for (j in 1:5){
    if (j == k){
      ...
      JJ[k,j]<-coreJaccard(tempphy,sample_data(tempphy)$Habitat,core_fraction=0.49,selection='shade',increase_cutoff=20,max_tax=20)
      UU[k,j]<-coreUniFrac(tempphy,sample_data(tempphy)$Habitat,core_fraction=0.49,selection='shade',increase_cutoff=20,initial_branches=2,max_tax=20)
      UUt[k,j]<-coreUniFrac(tempphy,sample_data(tempphy)$Habitat,core_fraction=0.49,mode='tip',selection='shade',increase_cutoff=20,max_tax=20)
    }
  }
}
